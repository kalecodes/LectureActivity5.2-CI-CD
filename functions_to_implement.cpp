...
// String based (at least 1)
// Splits a single string on separator into a vector of strings
std::vector<std::string> Split(std::string whole, std::string separator) 
{
	std::vector<std::string> parts;
	int start = 0;
	size_t sep_pos = whole.find(separator);

	while (sep_pos != std::string::npos) {
		parts.push_back(whole.substr(start, sep_pos - start));
		// set start to after the current separator
		start = sep_pos + separator.length();
		// find the next one after the new start position
		sep_pos = whole.find(separator, start);
	}

	// add remainder after last separator
	parts.push_back(whole.substr(start));
...
std::string RemoveAllSubstrings(std::string s1, std::string s2)
{
	std::string result = "";
	int start = 0;
	size_t s2_pos = s1.find(s2);

	while (s2_pos != std::string::npos) {
		result += s1.substr(start, s2_pos - start);
		// set start to after the current s2
		start = s2_pos + s2.length();
		// find the next one after the new start position
		s2_pos = s1.find(s2, start);
	}

	result += s1.substr(start);
	return result;
}


// Vector based (at least 2)
// Joins all strings in a vector together, using the glue string in between them
std::string Join(std::vector<std::string> pieces, std::string glue)
{
